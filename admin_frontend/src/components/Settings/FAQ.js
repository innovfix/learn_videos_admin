...
import * as action from '../../Action/FAQ/FAQ_Action';
...
const FAQ = () => {
	...
	const updateFAQLoading = useSelector(state => state.UpdateFAQReducer);
	const addFAQLoading = useSelector(state => state.AddFAQReducer);
    ...
    const [reportReasonData, setFAQData] = useState({
        id: 0,
        question: '',
        answer: ''
    });
    const dataTableInstanceRef = useRef(null);
    const questionRefFocus = useRef(null);
    const answerRefFocus = useRef(null);
            ...
            if ($ && tableRefCurrent) {
                dataTableInstanceRef.current = $(tableRefCurrent).DataTable({
                    ...
                    ajax: (dataTablesParams, callback) => {
                        ...
                        dispatch(action.getFAQ(params)).then((action) => {
                ...
                $(tableRefCurrent).on('click', '.delete-btn', function (e) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            dispatch(action.deleteFAQ({id: rowData.id})).then((action) => {
                                Swal.fire({
                                    title: "Deleted!",
    ...
    const onChange = (e) => {
        setFAQData({ ...reportReasonData, [e.target.name]: e.target.value })
    }
    ...
    const addFAQData = (e) => {
        ...
        setErrors({});
        let customErrors = {};
        if (reportReasonData.question === '') {
            customErrors = { ...customErrors, name: "Please Enter Question" }
            questionRefFocus.current.focus();
        }
        if (reportReasonData.answer === '') {
            customErrors = { ...customErrors, name: "Please Enter Answer" }
            answerRefFocus.current.focus();
        }
        ...
        var data = {
            'question': reportReasonData.question,
            'answer': reportReasonData.answer,
        };

        if(reportReasonData.id !== 0){
            data.id = reportReasonData.id;
            dispatch(action.updateFAQ(data)).then((response) => {
            ...
            dispatch(action.addFAQ(data)).then((response) => {
                toast.success(response.responseMessage);
                closeModel('Add');
                            ...
                            <div className="card-header">
                                <div className="page-title w-100 d-flex align-items-center justify-content-between">
                                    <div className="d-flex align-items-center justify-content-between">
                                        <h4>FAQ (<span className="total_user">{<NumberDisplay count={count} />}</span>)</h4>
                                    </div>
                                    <button className="btn upgrade-btn add-languages-btn" onClick={(e) => openModel('Add')}> Add FAQ </button>
                                </div>
                            </div>
                            <div className="card-body">
                                <table ref={tableRef} className="genres-table user-table table-layout-fixed table table-striped w-100 dataTable no-footer" id="languages-Contents" role="grid"></table>
                            </div>
                        </div> 
                    </section>
                        ...
                        showModel.show && (
                            <Model className="common-popup add-languages-popup active">
                                ...
                                <div className="edit-inner form-img-upload">
                                    <form onSubmit={addFAQData}>
                                        <div className="row">
                                            <div className="col-12"> 
                                                <div className="form-group">
                                                    <label htmlFor="question">Question</label>
                                                    <input type="text" id="question" name="question" placeholder="Enter Title" ref={questionRefFocus} onChange={onChange} value={reportReasonData.question} />
                                                    <span className='text-danger pt-2'>{errors?.question}</span>
                                                </div>
                                            </div>
                                            <div className="col-12"> 
                                                <div className="form-group">
                                                    <label htmlFor="answer">Answer</label>
                                                    <input type="text" id="answer" name="answer" placeholder="Enter Title" ref={answerRefFocus} onChange={onChange} value={reportReasonData.answer} />
                                                    <span className='text-danger pt-2'>{errors?.answer}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="action-footer m-0">
                                                <button type="submit" className="btn" disabled={showModel.title === 'Add' ? addFAQLoading.loading : updateFAQLoading.loading}>
                                                    {(showModel.title === 'Add' ? addFAQLoading.loading : updateFAQLoading.loading) ? (
