...
import * as action from '../../Action/ReportReason/ReportReason_Action';
...
const ReportReason = () => {
	...
	const updateReportReasonLoading = useSelector(state => state.UpdateReportReasonReducer);
	const addReportReasonLoading = useSelector(state => state.AddReportReasonReducer);
            ...
            if ($ && tableRefCurrent) {
                dataTableInstanceRef.current = $(tableRefCurrent).DataTable({
                    ...
                    ajax: (dataTablesParams, callback) => {
                        ...
                        dispatch(action.getReportReason(params)).then((action) => {
                    ...
                    columns: [
                        ...
                        { data: 'reason_title', title: "Reson" , width: "" ,  className: "report-column-name" },
                        {
                            data: null,
                            title: 'Action',
                            // orderable: false,
                ...
                $(tableRefCurrent).on('click', '.edit-btn', function (e) {
                    ...
                    setReportReasonData({
                        id: rowData.id,
                        reason_title: rowData.reason_title
                    });
                });
                ...
                $(tableRefCurrent).on('click', '.delete-btn', function (e) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            dispatch(action.deleteReportReason({id: rowData.id})).then((action) => {
                                Swal.fire({
                                    title: "Deleted!",
    ...
    const onChange = (e) => {
        setReportReasonData({ ...reportReasonData, [e.target.name]: e.target.value })
    }

    const addReportReasonData = (e) => {
        e.preventDefault();
        const isAccessAllowed = user?.login_type !== 'Guest';
        ...
        setErrors({});
        let customErrors = {};
        if (reportReasonData.reason_title === '') {
            customErrors = { ...customErrors, name: "Please Enter Reason" }
        ...
        var data = {
            'reason_title': reportReasonData.reason_title,
        };

        if(reportReasonData.id !== 0){
            data.id = reportReasonData.id;
            dispatch(action.updateReportReason(data)).then((response) => {
            ...
            dispatch(action.addReportReason(data)).then((response) => {
                toast.success(response.responseMessage);
                closeModel('Add');
                            ...
                            <div className="card-header">
                                <div className="page-title w-100 d-flex align-items-center justify-content-between">
                                    <div className="d-flex align-items-center justify-content-between">
                                        <h4>Report Reason (<span className="total_user">{<NumberDisplay count={count} />}</span>)</h4>
                                    </div>
                                    <button className="btn upgrade-btn add-languages-btn" onClick={(e) => openModel('Add')}> Add Report Reason </button>
                                </div>
                            </div>
                            <div className="card-body">
                                <table ref={tableRef} className="genres-table user-table table table-layout-fixed table-striped w-100 dataTable no-footer" id="languages-Contents" role="grid"></table>
                            </div>
                        </div> 
                    </section>
                        ...
                        showModel.show && (
                            <Model className="common-popup add-languages-popup active">
                                <div className="edit-heading d-flex align-items-center justify-content-between">
                                    <h3>{showModel.title} ReportReason</h3>
                                ...
                                <div className="edit-inner form-img-upload">
                                    <form onSubmit={addReportReasonData}>
                                        <div className="row">
                                            <div className="col-12"> 
                                                <div className="form-group">
                                                    <label htmlFor="reason_title">Title</label>
                                                    <input type="text" id="reason_title" name="reason_title" placeholder="Enter Title" ref={nameRefFocus} onChange={onChange} value={reportReasonData.reason_title} />
                                            ...
                                            <div className="action-footer m-0">
                                                <button type="submit" className="btn" disabled={showModel.title === 'Add' ? addReportReasonLoading.loading : updateReportReasonLoading.loading}>
                                                    {(showModel.title === 'Add' ? addReportReasonLoading.loading : updateReportReasonLoading.loading) ? (
