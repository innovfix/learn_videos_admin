...
class DeleteTypesController {

    static async deleteTypes(event, context) {
        ...
        try {
            // Validate input data
            let response = await DeleteTypesController.deleteTypesValidator(request);
            if (Object.keys(response).length > 0) {
                return context.status(400).send(response);
            }
            
            // Perform add operation
            const deleteTypes = await DeleteTypesController.deleteTypesOperation(
                request,
                corelationId,
                context,
                authorizer
    ...
    static async deleteTypesOperation(data, corelationId, context, authorizer) {
        try {
    ...
    static async deleteTypesValidator(req) {
        let errObj = {};
...
module.exports = DeleteTypesController;
