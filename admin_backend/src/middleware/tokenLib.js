...
const encrypt = (textString, salt) => {
  const hashedPassword = crypto.createHmac(ALGO, salt).update(textString).digest('hex');
...
const validateAppToken = function (token) {
  try {
    let decoded = jwt.verify(token, SHA256(process.env.JWT_SECRET).toString());
    return decoded;
  } catch (error) {
    return false;
  }
