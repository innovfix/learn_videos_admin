...
import * as action from '../Action/Categories/Categories_Action';
...
const Genres = () => {
	...
	const updateCategoriesLoading = useSelector(state => state.UpdateCategoriesReducer);
	const addCategoriesLoading = useSelector(state => state.AddCategoriesReducer);
            ...
            if ($ && tableRefCurrent) {
                dataTableInstanceRef.current = $(tableRefCurrent).DataTable({
                    ...
                    ajax: (dataTablesParams, callback) => {
                        ...
                        dispatch(action.getCategories(params)).then((action) => {
                ...
                $(tableRefCurrent).on('click', '.delete-btn', function (e) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            dispatch(action.deleteCategories({id: rowData.id})).then((action) => {
                                Swal.fire({
                                    title: "Deleted!",
    ...
    const addCategoryData = (e) => {
        ...
        if(categoryData.id !== 0){
            ...
            dispatch(action.updateCategories(data)).then((response) => {
            ...
            dispatch(action.addCategories(data)).then((response) => {
                toast.success(response.responseMessage);
                closeModel('Add');
                            ...
                            <div className="card-header">
                                <div className="page-title w-100 d-flex align-items-center justify-content-between">
                                    <div className="d-flex align-items-center justify-content-between">
                                        <h4>Genres (<span className="total_user">{<NumberDisplay count={count} />}</span>)</h4>
                                    </div>
                                    <button className="btn upgrade-btn add-genre-btn" onClick={(e) => openModel('Add')}> Add Genres </button>
                                </div>
                            </div>
                            <div className="card-body">
                                <table ref={tableRef} className="genres-table user-table table table-striped w-100 dataTable no-footer" id="genres-Contents" role="grid"></table>
                            </div>
                        </div> 
                    </section>
                                        ...
                                        <div className="row">
                                            <div className="col-12"> 
                                                <div className="form-group">
                                                    ...
                                                    <input type="text" id="name" name="name" value={categoryData.name} onChange={onChange} placeholder="Enter name" ref={nameRefFocus} />
                                                    <span className='text-danger pt-2'>{errors?.name}</span>
                                                </div>
                                                <div className="action-footer">
													<button type="submit" className="btn" disabled={showModel.title === 'Add' ? addCategoriesLoading.loading : updateCategoriesLoading.loading}>
														{(showModel.title === 'Add' ? addCategoriesLoading.loading : updateCategoriesLoading.loading) ? (
