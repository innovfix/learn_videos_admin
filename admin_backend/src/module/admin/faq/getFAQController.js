...
class GetFAQController {
    ...
    static async getFAQOperation(req, corelationId, context, authorizer) {
        try {
            const { search, start, length, dir, sortColumn, getAll } = req;

            const searchValue = search || '';
            const orderBy = dir || 'asc';
            const column = sortColumn || 'question';
            ...
            if (searchValue) {
                query += ` WHERE (question LIKE ? OR question LIKE ?) `;
            ...
            const [count] = await db.query(`SELECT COUNT(*) as total FROM faqs`);

            const [filteredCount] = await db.query(`SELECT COUNT(*) as total FROM faqs WHERE (question LIKE ? OR question LIKE ?)`, [`%${searchValue}%`, `%${searchValue}%`]);
