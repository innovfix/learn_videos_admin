...
class UpdateNotificationController {
    ...
    static async updateNotification(event, context) {
        ...
        try {
            // Validate input data
            let response = await UpdateNotificationController.updateNotificationValidator(event);
            if (Object.keys(response).length > 0) {
                return context.status(400).send(response);
            }
            
            // Perform add operation
            const addNotification = await UpdateNotificationController.updateNotificationOperation(
                event,
                corelationId,
                context,
                authorizer
    ...
    static async updateNotificationOperation(req, corelationId, context, authorizer) {
        try {
    ...
    static async updateNotificationValidator(req) {
        let errObj = {};
        if (!req?.body?.type_id) {
            return {
...
module.exports = UpdateNotificationController;
