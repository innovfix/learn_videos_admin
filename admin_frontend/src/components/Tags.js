...
import * as action from '../Action/Tags/Tags_Action';
...
const Tags = () => {
	...
	const updateTagsLoading = useSelector(state => state.UpdateTagsReducer);
	const addTagsLoading = useSelector(state => state.AddTagsReducer);
            ...
            if ($ && tableRefCurrent) {
                dataTableInstanceRef.current = $(tableRefCurrent).DataTable({
                    ...
                    ajax: (dataTablesParams, callback) => {
                        ...
                        dispatch(action.getTags(params)).then((action) => {
                ...
                $(tableRefCurrent).on('click', '.delete-btn', function (e) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            dispatch(action.deleteTags({id: rowData.id})).then((action) => {
                                Swal.fire({
                                    title: "Deleted!",
    ...
    const addTagsData = (e) => {
        ...
        if(tagsData.id !== 0){
            data.id = tagsData.id;
            dispatch(action.updateTags(data)).then((response) => {
            ...
            dispatch(action.addTags(data)).then((response) => {
                toast.success(response.responseMessage);
                closeModel('Add');
                            ...
                            <div className="card-header">
                                <div className="page-title w-100 d-flex align-items-center justify-content-between">
                                    <div className="d-flex align-items-center justify-content-between">
                                        <h4>Tags (<span className="total_user">{<NumberDisplay count={count} />}</span>)</h4>
                                    </div>
                                    <button className="btn upgrade-btn add-genre-btn" onClick={(e) => openModel('Add')}> Add Tag </button>
                                </div>
                            </div>
                        ...
                        showModel.show && (
                            <Model className="common-popup add-genre-popup active">
                                ...
                                <div className="edit-inner form-img-upload">
                                    <form onSubmit={addTagsData}>
                                        <div className="row">
                                            <div className="col-12"> 
                                                <div className="form-group">
                                                    <label htmlFor="name">Name</label>
                                                    <input type="text" id="name" name="name" value={tagsData.name} onChange={onChange} placeholder="Enter name" ref={nameRefFocus} />
                                                    <span className='text-danger pt-2'>{errors?.name}</span>
                                                </div>
                                                <div className="action-footer">
													<button type="submit" className="btn" disabled={showModel.title === 'Add' ? addTagsLoading.loading : updateTagsLoading.loading}>
														{(showModel.title === 'Add' ? addTagsLoading.loading : updateTagsLoading.loading) ? (
